...
const createMockRequest = (
  ...
  return {
    ...
    url: params
      ? `http://localhost/api/salary/${params.recordId}`
      : "http://localhost/api/salary",
...
describe("SalaryRecord API Integration Tests", () => {
  ...
  describe("PUT /api/salary/{recordId}", () => {
    ...
    it("should return 400 for invalid input (e.g., negative baseSalary in update)", async () => {
      ...
      const invalidUpdate = {
        ...
        netPay: 100,
        baseSalary: -50,
      }; // Ensure all required fields for validation
  ...
  describe("DELETE /api/salary/{recordId}", () => {
    ...
    it("should return 401 if unauthorized", async () => {
      const request = createMockRequest("DELETE", undefined, undefined, {
        recordId,
      }); // No token
      const response = await deleteSalaryDELETE(request, {
        params: { recordId },
