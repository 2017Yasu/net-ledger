...
test.describe('Salary History and Detail Flow', () => {
  ...
  test.beforeAll(async () => {
    ...
    const secondRecordYear = createdRecordYear + (secondRecordMonth < createdRecordMonth ? 1 : 0)
  ...
  test('should display salary history and navigate to detail view', async ({ page }) => {
    // 1. Navigate to the "History" page
    await page.goto('/salary/history')
    await expect(page.locator('h1', { hasText: 'Salary History' })).toBeVisible()

    // 2. Verify that all created records are displayed in the list
    await expect(page.locator(`text=${createdRecordMonth}/${createdRecordYear}`)).toBeVisible()
    await expect(page.locator(`text=Net: $2000.00`)).toBeVisible()
    // Make sure the second record is also visible
    const month2 = (new Date().getMonth() % 12) + 1 + 1; // Adjust if first record was Dec
    const year2 = (new Date().getMonth() + 1 + 1 > 12) ? new Date().getFullYear() + 1 : new Date().getFullYear();
    await expect(page.locator(`text=${(createdRecordMonth % 12) + 1}/${createdRecordYear + (secondRecordMonth < createdRecordMonth ? 1 : 0)}`)).toBeVisible()
    ...
    await expect(page.locator('h2', { hasText: `Salary Record for ${createdRecordMonth}/${createdRecordYear}` })).toBeVisible()
  ...
  test('should allow user to edit a salary record', async ({ page }) => {
    ...
    await expect(page.locator('h1', { hasText: 'Edit Salary Record' })).toBeVisible()
    ...
    await expect(page.locator('h2', { hasText: `Salary Record for ${createdRecordMonth}/${createdRecordYear}` })).toBeVisible() // Back to detail view
  ...
  test('should allow user to delete a salary record', async ({ page }) => {
    ...
    const listItem = page.locator(`a[href="/salary/history/${secondRecordId}"]`).locator('..')
    await listItem.locator('[aria-label="delete"]').click()

    // Confirm deletion in the alert dialog
    page.on('dialog', async (dialog) => {
      expect(dialog.message()).toContain('Are you sure you want to delete this salary record?')
      await dialog.accept()
    })

    // Verify the record is no longer visible
    await expect(page.locator(`text=${(createdRecordMonth % 12) + 1}/${createdRecordYear + (secondRecordMonth < createdRecordMonth ? 1 : 0)}`)).not.toBeVisible()
