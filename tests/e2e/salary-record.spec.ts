...
test.describe("Salary Record Flow", () => {
  ...
  test("should allow a user to create and update a salary record", async ({
    page,
  }) => {
    // 1. Navigate to the "Record Salary" page
    await page.goto("/salary/record");
    await expect(
      page.locator("h1", { hasText: "Create Salary Record" }),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Salary record created successfully!"),
    ).toBeVisible();
