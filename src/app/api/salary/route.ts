...
export async function POST(request: Request) {
  try {
    ...
    if (existingRecord) {
      return NextResponse.json({ message: 'Salary record for this month and year already exists. Please update the existing record.' }, { status: 409 })
    }
    ...
    const salaryRecord = await prisma.salaryRecord.create({
      data: {
        ...
        overtimeAllowance: overtimeAllowance ? new Decimal(overtimeAllowance) : null,
        commutingAllowance: commutingAllowance ? new Decimal(commutingAllowance) : null,
        otherAllowances: otherAllowances ? new Decimal(otherAllowances) : null,
        grossEarnings: new Decimal(grossEarnings),
        socialInsuranceContributions: socialInsuranceContributions ? new Decimal(socialInsuranceContributions) : null,
        ...
        yearEndTaxAdjustment: yearEndTaxAdjustment ? new Decimal(yearEndTaxAdjustment) : null,
        paidTimeOffDaysUsed,
        paidTimeOffDaysRemaining,
      },
    })
...
export async function GET(request: Request) {
  try {
    ...
    if (!salaryRecords || salaryRecords.length === 0) {
        return NextResponse.json({ message: 'No salary records found' }, { status: 404 })
    }
