...
export default function SalaryDetailPage({ params }: SalaryDetailPageProps) {
  ...
  return (
    <Container maxWidth="md">
      <Box sx={{ mt: 4 }}>
        {isEditing ? (
          <SalaryForm initialData={salaryRecord} onSubmit={handleUpdate} loading={loading} error={error} isEdit={true} />
        ) : (
          <>
            <SalaryDetailView salaryRecord={salaryRecord} />
            <Button variant="contained" onClick={() => setIsEditing(true)} sx={{ mt: 3, mr: 2 }}>
              Edit Record
            </Button>
            <Button variant="outlined" onClick={() => router.push('/salary/history')} sx={{ mt: 3 }}>
              Back to History
            </Button>
          </>
        )}
